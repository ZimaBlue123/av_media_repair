...
Goal: Repair "unplayable" video files into playable ones without overwriting the original files.
...
```bash
python -m video_repair.cli probe "E:\Cursor Project\recording1-20260401.mp4"
```
...
> - `untrunc.exe` (on Windows, this project can auto-download and cache to `module1_video_repair/tools/`)

```bash
python -m video_repair.cli untrunc "E:\Cursor Project\good.mp4" "E:\Cursor Project\recording1-20260401.mp4" -o "E:\Cursor Project\output_fixed.mp4" --untrunc "C:\path\to\untrunc.exe"
```

### 4) Option B (High Success Rate): Batch Rebuild moov using "Normal Same-Settings Video"
...
One-click batch repair + self-check (writes JSON report):
...
- If there are multiple videos in `template` directory, the tool automatically selects the **largest** one as the sample.
- On Windows, if `untrunc/ffprobe` is not installed locally, it will try to download from GitHub Releases and cache to `module1_video_repair/tools/` (reused next time).
